...
/**
 * Process fallback archetype data when database data is not available
 */
export const processFallbackData = (
  archetypeId: ArchetypeId,
  getArchetypeEnhanced: any,
  getFamilyById: any
) => {
  console.log(`Processing fallback data for archetype ${archetypeId}`);
  
  // Get static data from the local dataset
  const staticData = getArchetypeEnhanced?.(archetypeId);
  ...
  if (!staticData) {
    ...
    return {
      archetypeData: null,
      familyData: null,
      dataSource: 'No fallback data available'
    };
  }
  
  // Extract family information
  const familyId = staticData.familyId || staticData.family_id || 'unknown';
  const family = getFamilyById?.(familyId);
  
  // Normalize the property names
  const normalizedData = normalizePropertyNames(staticData);
  
  return {
    archetypeData: {
      ...normalizedData,
      id: archetypeId,
      name: staticData.name || staticData.archetype_name || archetypeId,
      familyId: familyId,
      familyName: staticData.familyName || staticData.family_name || (family?.name || ''),
      hexColor: staticData.hexColor || staticData.hex_color || '#6E59A5'
    },
    familyData: family,
    dataSource: 'Static Data (Fallback)'
  };
};
